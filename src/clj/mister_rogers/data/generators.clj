...
;
;   Adapted by Mark Engelberg to use ThreadLocalRandom without dynamic var

(ns ^{:author "Stuart Halloway"
      :doc "Data generators for Clojure."}
    mister-rogers.data.generators
...
(defn geometric
  ...
  ^long [p]
  (core/long (Math/ceil (/ (Math/log (.nextDouble (ThreadLocalRandom/current)))
                           (Math/log (- 1.0 p))))))

(defn uniform
  "Uniform distribution from lo (inclusive) to hi (exclusive).
   Defaults to range of Java long."
  (^long [] (.nextLong (ThreadLocalRandom/current)))
  (^long[lo hi] {:pre [(< lo hi)]}
         (clojure.core/long (Math/floor (+ lo (* (.nextDouble (ThreadLocalRandom/current)) (- hi lo)))))))

(defn float
  "Generate a float between 0 and 1 based on (ThreadLocalRandom/current)"
  ^double []
  (.nextFloat (ThreadLocalRandom/current)))

(defn double
  "Generate a double between 0 and 1 based on (ThreadLocalRandom/current)."
  ^double []
  (.nextDouble (ThreadLocalRandom/current)))

(defn rand-nth
  "Replacement of core/rand-nth that allows control of the
   randomization basis (through binding (ThreadLocalRandom/current))."
  [coll]
...
(def long
  "Returns a long based on (ThreadLocalRandom/current). Same as uniform."
  uniform)
...
(defn int
  "Returns a int based on (ThreadLocalRandom/current) in the int range."
  []
  (uniform Integer/MIN_VALUE (inc Integer/MAX_VALUE)))

(defn short
  "Returns a short based on (ThreadLocalRandom/current) in the short range."
  []
  (uniform Short/MIN_VALUE (inc (core/long Short/MAX_VALUE))))

(defn byte
  "Returns a long based on (ThreadLocalRandom/current) in the byte range."
  ^long []
  (uniform Byte/MIN_VALUE (inc (core/int Byte/MAX_VALUE))))

(defn boolean
  "Returns a bool based on (ThreadLocalRandom/current)."
  []
  (.nextBoolean (ThreadLocalRandom/current)))

(defn printable-ascii-char
  "Returns a char based on (ThreadLocalRandom/current) in the printable ascii range."
  []
  (core/char (uniform 32 127)))

(defn char
  "Returns a character based on (ThreadLocalRandom/current) in the range 0-65536."
  []
...
(defn scalar
  "Returns a scalar based on (ThreadLocalRandom/current)."
  []
...
(defn collection
  "Returns a collection of scalar elements based on (ThreadLocalRandom/current)."
  []
  (let [[coll args] (rand-nth collections)]
    (apply coll (map rand-nth args))))

(defn anything
  "Returns a scalar or collection based on (ThreadLocalRandom/current)."
  []
...
(defn shuffle
  "Shuffle coll based on (ThreadLocalRandom/current)"
  [coll]
  ;; using our own fischer-yates instead of core/shuffle so that
  ;; we'll get the same shuffle, given the same (ThreadLocalRandom/current).
  (fisher-yates coll))

(defn reservoir-sample
  "Reservoir sample ct items from coll, using (ThreadLocalRandom/current)."
  [ct coll]
